<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Create New Inventory Movement') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h1 class="text-2xl font-semibold text-gray-700 dark:text-gray-300 mb-6">Create Inventory Movement</h1>

                    <form action="{{ route('im.store') }}" method="POST" class="space-y-6">
                        @csrf
                            ...
                            <select name="Product_ID" id="Product_ID" required
                                ...
                                @foreach ($product as $prod)
                                    <option value="{{ $prod->id }}" {{ old('Product_ID') == $prod->id ? 'selected' : '' }}>
                                        {{ $prod->id }} - {{ $prod->Name ?? 'N/A' }} {{-- Assuming Product model has a 'Name' attribute --}}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="Movement_Type" id="Movement_Type" value="{{ old('Movement_Type') }}" required
                                   class="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 dark:text-gray-100"
                        ...
                        <div>
                            ...
                            <select name="User_ID" id="User_ID" required
                                ...
                                @foreach ($User_ID as $user) {{-- Changed variable name from $User_ID to $user for clarity --}}
                                    <option value="{{ $user->id }}" {{ old('User_ID') == $user->id ? 'selected' : '' }}>
                                        {{ $user->id }} - {{ $user->name ?? 'N/A' }} {{-- Assuming User model has a 'name' attribute --}}
                                    </option>
                                @endforeach
                            </select>
                        </div>
