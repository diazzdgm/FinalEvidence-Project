        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h1 class="text-2xl font-semibold text-gray-700 dark:text-gray-300 mb-6">Create Product</h1>
                            ...
                            <select name="Warehouse" id="Warehouse" required
                                ...
                                @foreach ($WH as $warehouse)
                                    <option value="{{ $warehouse->id }}" {{ old('Warehouse') == $warehouse->id ? 'selected' : '' }}>
                                        {{ $warehouse->id }} - {{ $warehouse->Name ?? 'N/A' }} {{-- Assuming Warehouse model has a 'Name' attribute --}}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="Category_ID" id="Category_ID" required
                                ...
                                @foreach ($CatID as $category)
                                    <option value="{{ $category->id }}" {{ old('Category_ID') == $category->id ? 'selected' : '' }}>
                                        {{ $category->id }} - {{ $category->Name ?? 'N/A' }} {{-- Assuming Category model has a 'Name' attribute --}}
                                    </option>
                                @endforeach
                            </select>
                        </div>
